...
const DEVSERVER_OPTIONS = {
    ...
    "client": {
        ...
        // "progress": true, // XXX error, when two dev servers used
        "overlay": {
            "errors": true,
            "warnings": false,
            "runtimeErrors": true,
...
export default class WebpackRunner {
    ...
    constructor ( { mode, command, useAnalyzer, useCache, buildCordova, listen, httpsPort, httpPort, proxytUrl } = {} ) {
        ...
        if ( this.#command === "dev" ) {
            ...
            this.#httpDevServerOptions = this.#buildDevServerOptions( {
    ...
    async #runDevServer ( webpackConfigs ) {
        ...
        this.#createDevServerProxy( this.#httpDevServerOptions );
        this.#createDevServerProxy( this.#httpsDevServerOptions );

        const compiler = webpack( webpackConfigs ),
            httpServer = new WebpackDevServer( this.#httpDevServerOptions, compiler ),
            httpsServer = new WebpackDevServer( this.#httpsDevServerOptions, compiler );
