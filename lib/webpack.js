...
const DEVSERVER_OPTIONS = {
    ...
    DEVSERVER_OPTIONS_HTTP = {
        ...JSON.parse( JSON.stringify( DEVSERVER_OPTIONS ) ),
        "port": 80,
    },
    DEVSERVER_OPTIONS_HTTPS = {
        ...JSON.parse( JSON.stringify( DEVSERVER_OPTIONS ) ),
        "port": 443,
        "https": true,
    };
...
export default class WebpackRunner {
    ...
    constructor ( { mode, command, useAnalyzer, useCache, buildCordova, listen, httpsPort, httpPort, proxytUrl } = {} ) {
        ...
        if ( this.#command === "dev" ) {
            ...
            if ( listen ) DEVSERVER_OPTIONS_HTTP.host = listen;
            if ( listen ) DEVSERVER_OPTIONS_HTTPS.host = listen;

            if ( httpPort ) DEVSERVER_OPTIONS_HTTP.port = httpPort;
            if ( httpsPort ) DEVSERVER_OPTIONS_HTTPS.port = httpsPort;

            if ( proxytUrl ) this.#devServerProxyUrl = proxytUrl;

            if ( this.isProduction ) {
                DEVSERVER_OPTIONS_HTTP.client.overlay.runtimeErrors = false;
                DEVSERVER_OPTIONS_HTTPS.client.overlay.runtimeErrors = false;
            }
        }
        else {
    ...
    async #runDevServer ( webpackConfigs ) {
        ...
        this.#createDevServerProxy( DEVSERVER_OPTIONS_HTTP );
        this.#createDevServerProxy( DEVSERVER_OPTIONS_HTTPS );

        const compiler = webpack( webpackConfigs ),
            httpServer = new WebpackDevServer( DEVSERVER_OPTIONS_HTTP, compiler ),
            httpsServer = new WebpackDevServer( DEVSERVER_OPTIONS_HTTPS, compiler );
    ...
    #printCompilationReport ( error ) {
        ...
        if ( this.#command === "dev" ) {
            ...
            info.push( `Webpack dev. server listening at: ${ansi.hl( `http://${DEVSERVER_OPTIONS_HTTP.host}:${DEVSERVER_OPTIONS_HTTP.port}/` )}` );

            info.push( `Webpack dev. server listening at: ${ansi.hl( `https://${DEVSERVER_OPTIONS_HTTPS.host}:${DEVSERVER_OPTIONS_HTTPS.port}/` )}` );
        }
