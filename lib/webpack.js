...
export default class WebpackRunner {
    ...
    constructor ( { mode, command, useAnalyzer, useCache, buildCordova, listen, httpsPort, httpPort, proxyUrl } = {} ) {
    ...
    async #runDevServer ( webpackConfigs ) {
        ...
        // start dev server
        await this.#webpackDevServer.start( compiler, {
            "context": this.context,
            "proxyUrl": this.env.build?.webpackDevServer?.proxyUrl,
            "httpsDomain": this.env.build?.webpackDevServer?.httpsDomain,
            "cert": this.env.build?.webpackDevServer?.cert,
            "key": this.env.build?.webpackDevServer?.key,
        } );
    }
    ...
    #printCompilationReport ( error ) {
        ...
        // devserver info
        if ( this.#command === "dev" ) {
            info.push( `Webpack dev. server listening at: ${ansi.hl( this.#webpackDevServer.httpUrl )}` );

            info.push( `Webpack dev. server listening at: ${ansi.hl( this.#webpackDevServer.httpsUrl )}` );

            if ( this.#webpackDevServer.httpsDomainUrl ) {
                info.push( `Webpack dev. server listening at: ${ansi.hl( this.#webpackDevServer.httpsDomainUrl )}` );
            }

            if ( this.#webpackDevServer.proxyUrl ) {
                info.push( `\nWebpack dev. server proxies requests to: ${ansi.hl( this.#webpackDevServer.proxyUrl )}` );
            }
        }
