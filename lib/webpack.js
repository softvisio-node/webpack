...
export default class WebpackRunner {
    ...
    #printCompilationReport ( error ) {
        ...
        if ( this.#command === "dev" ) {
            ...
            info.push( `Webpack dev. server listening at: ${ansi.hl( `http://${DEVSERVER_OPTIONS.host}:${DEVSERVER_OPTIONS.port}/` )}` );
        }
        ...
        // error
        if ( error ) {
            console.log( ansi.hl( "• Targets: " ) + this.buildTargets + ", " + ansi.hl( "compilation status: " ) + ansi.error( ` FAIL ` ) );
        }

        // ok
        else {
            console.log( ansi.hl( "• Targets: " ) + this.buildTargets + ", " + ansi.hl( "compilation status: " ) + ansi.ok( ` SUCCESS ` ) );
        }
    }
