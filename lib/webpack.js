...
export default class WebpackRunner {
    ...
    async #buildWebpackConfigs () {
        if ( !this.#webpackConfigs ) {
            ...
            const appEnvJson = JSON.stringify( await this.#getAppEnv() ),
                appConfigJson = JSON.stringify( this.env.config ),
            ...
            for ( const dirent of fs.readdirSync( path.join( this.context, "resources/webpack-components" ), { "withFileTypes": true } ) ) {
                ...
                const Component = ( await import( url.pathToFileURL( path.join( this.context, "resources/webpack-components", dirent.name, "index.js" ) ) ) ).default;

                const component = new Component( this, dirent.name, {
            ...
            for ( const component of this.#components ) {
                ...
                if ( !this.#useCache ) {
                    ...
                    hash.update( JSON.stringify( component.webpackPreprocessorOptions ) );

                    component.webpackConfig.cache = {
                        ...component.webpackConfig.cache,
                        "name": path.join( this.buildTag, component.name ),
