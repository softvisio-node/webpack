...
export default class WebpackComponent {
    ...
    _validateEnv ( onv, location ) {
        ...
        if ( fs.existsSync( defaultEnvLocation ) ) {
            ...
            if ( defaultEnv.build ) onv.build = mergeObjects( {}, defaultEnv.build, onv.build );
            if ( defaultEnv.config ) onv.config = mergeObjects( {}, defaultEnv.config, onv.config );
        }
        ...
        if ( fs.existsSync( envSchemaLocation ) ) {
            ...
            // valudate build
            if ( ajv.getSchema( "build" ) && !ajv.validate( "build", onv.build ) ) {
                return ajv.errors;
            }

            // valudate config
            if ( ajv.getSchema( "config" ) && !ajv.validate( "config", onv.config ) ) {
                return ajv.errors;
            }
        }
    }
