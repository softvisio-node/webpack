...
export default class WebpackComponent {
    ...
    _validateEnv ( config, location ) {
        ...
        if ( fs.existsSync( defaultEnvLocation ) ) {
            ...
            if ( defaultEnv.build ) config.build = mergeObjects( [{}, defaultEnv.build, config.build] );
            if ( defaultEnv.config ) config.config = mergeObjects( [{}, defaultEnv.config, config.config] );
        }
        ...
        if ( fs.existsSync( envSchemaLocation ) ) {
            const ajv = new Ajv().addSchema( readConfig( envSchemaLocation ) );
            ...
            // valudate build
            if ( ajv.getSchema( "build" ) && !ajv.validate( "build", config.build ) ) {
                return ajv.errors;
            }

            // valudate config
            if ( ajv.getSchema( "config" ) && !ajv.validate( "config", config.config ) ) {
                return ajv.errors;
            }
        }
    }
