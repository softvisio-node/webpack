...
export default class DevServer {
    ...
    // public
    async start ( compiler, { context, proxyUrl, cert, key } = {} ) {
        this.#proxyUrl ??= proxyUrl;
        ...
        SSL: if ( cert && key ) {
            cert = path.resolve( context, cert );

            if ( !fs.existsSync( cert ) ) break SSL;

            key = path.resolve( context, key );

            if ( !fs.existsSync( key ) ) break SSL;

            this.#options.server.cert = cert;
            this.#options.server.key = key;
        }
