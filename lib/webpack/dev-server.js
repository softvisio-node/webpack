...
const defaultHttpsDomain = "local.softvisio.net",
    defaultHttpsCert = resolve( "#resources/local.softvisio.net/fullchain1.pem", import.meta.url ),
    defaultHttpsKey = resolve( "#resources/local.softvisio.net/privkey1.pem", import.meta.url );
    ...
    async start ( compiler, { proxyUrl, https = {} } = {} ) {
        ...
        try {
            if ( cert && key ) {
                ...
                if ( !fs.existsSync( cert ) ) throw Error();

                key = path.resolve( this.#webpack.context, key );

                if ( !fs.existsSync( key ) ) throw Error();
